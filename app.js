/* ─────────────────────────────────────────────────────────
   PersonalPTF — app.js
   ⚠️  IMPORTANTE: imposta SCRIPT_URL con il tuo URL Apps Script
───────────────────────────────────────────────────────── */

// ─── CONFIG ──────────────────────────────────────────────
const SCRIPT_URL = 'INCOLLA_QUI_IL_TUO_URL_APPS_SCRIPT';
...
window.addEventListener('DOMContentLoaded', () => {
  // Imposta data odierna nel form
  document.getElementById('f-date').value = new Date().toISOString().split('T')[0];
  // Aggiorna sommario form al cambio valori
  ...
  try {
    ...
    document.getElementById('loading').innerHTML = `
      <div style="padding:20px;width:100%;max-width:360px">
        ...
        <div class="error-banner">${e.message}\n\nControllare SCRIPT_URL in app.js</div>
...
function overview() {
  ...
  <div class="pnl-summary">
    <div>
      ...
      <div style="font-size:22px;font-weight:700;font-family:var(--mono);color:${pnlC};letter-spacing:-0.5px">
        ${(o.pnlEur||0)>=0?'+':''}${eur(o.pnlEur)}
      </div>
    </div>
    <div style="text-align:right">
      <div style="font-size:30px;font-weight:700;font-family:var(--mono);color:${pnlC};letter-spacing:-1px">
        ${(o.pnlEur||0)>=0?'+':''}${pnlPct}%
      </div>
...
function _miniPosizioni() {
  ...
  return `<div class="card">
    <div class="section-title">◆ Top Posizioni</div>
    ${rows}
  ...
  const rows = m.map(row => {
    ...
    return `<div style="margin-bottom:10px">
      ...
      <div class="bar-track">
        ...
        <div class="bar-target" style="left:${parseFloat(tgt)}%"></div>
      </div>
    </div>`;
  }).join('');
...
function performance() {
  ...
  if (!perf.length) return '<div class="empty">Nessun dato performance disponibile.<br>Il sistema registra ogni giorno alle 18:00.</div>';
  ...
  const labels = perf.filter((_,i)=>i%Math.max(1,Math.floor(perf.length/5))===0)
    .map(p=>`<span class="chart-label">${p.data}</span>`).join('');
  ...
  const benchRows = [
    ...
    <div class="bench-row">
      ...
      <span style="font-size:14px;font-weight:700;font-family:var(--mono);color:${r.color}">${r.val>=0?'+':''}${r.val.toFixed(2)}%</span>
    </div>`).join('');

  const hist = [...perf].reverse().slice(0,10).map(p=>{
    const t=(p.twr*100).toFixed(2);
    const c=parseFloat(t)>=0?'var(--green)':'var(--red)';
    return `<div class="bench-row">
      <span style="font-size:11px;color:var(--text3)">${p.data}</span>
      <span style="font-family:var(--mono);font-size:12px;font-weight:600;color:${c}">${parseFloat(t)>=0?'+':''}${t}%</span>
    </div>`;
  }).join('');
  ...
  <div class="card">
    <div class="perf-hero">
      ...
      <div class="perf-sub">Time-Weighted Return · ${perf.length} giorni · dal 21/04/2026</div>
    </div>
    <div class="chart-wrap">${chart}</div>
    <div class="chart-labels">${labels}</div>
    <div style="display:flex;gap:14px;margin-top:10px;font-size:10px;color:var(--text3)">
      <span>— <span style="color:var(--green)">●</span> TWR</span>
      ${msci?'<span>-- <span style="color:var(--blue)">●</span> MSCI</span>':''}
      ${sp?'<span>-- <span style="color:var(--violet)">●</span> S&P</span>':''}
    </div>
  </div>
...
function _buildChart(perf) {
  ...
  function pts(arr){return arr.map((v,i)=>{
  ...
  const lx=PAD+(li/Math.max(tv.length-1,1))*(W-PAD*2);
  const ly=H-PAD-((tv[li]-minV)/rng)*(H-PAD*2);
...
function posizioni() {
  ...
  const rows = sorted.map(p=>{
    ...
    return `<div class="pos-item">
      ...
      <div class="pos-right">
        <div class="pos-mv">${eur(p.mv)}</div>
        <div class="pos-pnl" style="color:${pc}">${pp>=0?'+':''}${pp.toFixed(2)}%</div>
        <div style="font-size:9px;color:var(--text3)">${bw.toFixed(1)}% ptf</div>
      </div>
    </div>`;
  }).join('');
...
function mandate() {
  ...
  const rows = m.map(row=>{
    ...
    return `<div class="mandate-row">
      ...
      <div class="bar-track">
        <div class="bar-fill" style="width:${bw.toFixed(1)}%;background:${fc}"></div>
        <div class="bar-target" style="left:${tw.toFixed(1)}%"></div>
      </div>
      <div class="mandate-nums">
        <span>Attuale: <b style="color:var(--text)">${(cur*100).toFixed(1)}%</b></span>
        <span>Target: <b style="color:var(--text)">${(tgt*100).toFixed(0)}%</b></span>
...
function transactions() {
  ...
  const rows = trx.map(t=>{
    ...
    return `<div class="trx-item">
      ...
      <div class="trx-info">
        <div class="trx-ticker">${t.ticker} <span class="trx-type" style="background:${tbg};color:${tc}">${t.tipo}</span></div>
        <div class="trx-date">${t.data} · ${(t.nome||'').slice(0,22)}</div>
      </div>
      <div class="trx-right">
        <div class="trx-amount" style="color:${ac}">${isBuy?'-':'+'}${amt}</div>
        ${t.pnlNetto&&typeof t.pnlNetto==='number'?`<div style="font-size:10px;color:var(--green);font-weight:600">P&L ${eur(t.pnlNetto)}</div>`:''}
      </div>
    </div>`;
  }).join('');
...
function simulatore() {
  ...
  <div class="card">
    <div class="slider-wrap">
      <div class="slider-label"><span>Capitale iniziale</span><span class="slider-val" id="lbl-cap">${eur(cap)}</span></div>
      <input type="range" id="sl-cap" min="5000" max="200000" step="1000" value="${Math.round(cap)}" oninput="updateSim()">
    </div>
    <div class="slider-wrap">
      <div class="slider-label"><span>PAC mensile</span><span class="slider-val" id="lbl-pac">€1.000</span></div>
      <input type="range" id="sl-pac" min="0" max="5000" step="100" value="1000" oninput="updateSim()">
    </div>
    <div class="slider-wrap">
      <div class="slider-label"><span>Anni</span><span class="slider-val" id="lbl-anni">10 anni</span></div>
      <input type="range" id="sl-anni" min="1" max="30" step="1" value="10" oninput="updateSim()">
    </div>
  </div>
  <div class="scenario-cards">
    <div class="scenario-card">
      <div class="scenario-label" style="color:var(--red)">⬇ Pess.</div>
      <div class="scenario-value" id="sc-pess" style="color:var(--red)">—</div>
      <div class="scenario-sub">+3%/anno</div>
    </div>
    <div class="scenario-card" style="border-color:rgba(59,130,246,0.25)">
      <div class="scenario-label" style="color:var(--blue)">→ Base</div>
      <div class="scenario-value" id="sc-base" style="color:var(--blue)">—</div>
      <div class="scenario-sub">+7%/anno</div>
    </div>
    <div class="scenario-card">
      <div class="scenario-label" style="color:var(--green)">⬆ Ott.</div>
      <div class="scenario-value" id="sc-opt" style="color:var(--green)">—</div>
      <div class="scenario-sub">+12%/anno</div>
    </div>
  </div>
  <div class="card">
    ...
    <div style="display:flex;gap:14px;margin-top:8px;font-size:10px;color:var(--text3)">
      <span><span style="color:var(--red)">●</span> Pessimistico</span>
      <span><span style="color:var(--blue)">●</span> Base</span>
      <span><span style="color:var(--green)">●</span> Ottimistico</span>
    </div>
  </div>`;
}
...
function updateFormSummary() {
  ...
  if (!qty || !price) {
    document.getElementById('form-summary').classList.remove('visible');
    return;
  }
  const totale = qty * price;
  const netto = FORM_TIPO === 'SELL' ? totale - comm : -(totale + comm);
  document.getElementById('form-summary').classList.add('visible');
  document.getElementById('form-summary').innerHTML =
    `${FORM_TIPO} ${qty} × ${ticker} @ €${price.toFixed(2)}\n` +
...
async function submitForm() {
  ...
  if (!ticker || !qty || !price || !date) {
    alert('Compila Ticker, Quantità, Prezzo e Data');
    return;
  }
  ...
  try {
    const res = await fetch(`${SCRIPT_URL}?action=addTransaction`, {
      method: 'POST',
      mode: 'cors',
    ...
    setTimeout(() => {
      ...
      btn.textContent = 'Conferma operazione';
